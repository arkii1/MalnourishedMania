...
public class RaycastController : MonoBehaviour
{
    ...
    [HideInInspector]
    public int staticTrapDirCase = 0; //0 = up, 1 = right, 2 = down, 3 = left
    #endregion
    ...
    public List<RaycastHit2D> GetAllCollisions(LayerMask hitMask, float rayLength)
    {
        ...
        //Remove dupes
        int index = hitList.Count - 1;
        while (index > 0)
        {
            if (hitList[index] == hitList[index - 1])
            {
                if (index < hitList.Count - 1)
                    (hitList[index], hitList[hitList.Count - 1]) = (hitList[hitList.Count - 1], hitList[index]);
